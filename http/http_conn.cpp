                     ...
                     int close_log, string user, string passwd, string sqlname)
{
    ...
    // 把客人的 sockfd 注册进 Epoll 管家的监听名单，并开启 EPOLLONESHOT（防并发冲突）。
...
void http_conn::init()
{
    // 无参的 init() 操纵的是频繁变化的请求状态，避免了为一个长连接上的多次请求反复去 new 和 delete 内存块
    //下一次同一个请求来临，发来的报文里带有 Connection: keep-alive，不能调用带参数的init(),会关闭sockfd，将状态机状态拨回 CHECK_STATE_REQUESTLINE，清空读写缓冲区数据即可
    mysql = NULL;
    bytes_to_send = 0;
    bytes_have_send = 0;
...
//第一步：当 epoll 提示网卡有数据时，不管是Proactor，还是Reactor，都会执行这个函数
//阶段二：读取请求报文。
// 客户端把包含 GET /index.html... 的 HTTP 报文通过网卡发了过来，触发 Epoll 的 EPOLLIN 读事件
bool http_conn::read_once()
{
    ...
    else
    {
        ...
        while (true)
        {
            ...
            //当网卡被抽干时，非阻塞的 recv() 不会傻等，而是立刻返回 -1，并甩给经理一个 EAGAIN 错误码。
...
bool http_conn::write()
{
    ...
    while (1)
    {
        ...
        {
            unmap();
            //如果所有数据（网页图片、HTML等）都发送完毕了，事件重新改回 EPOLLIN
            modfd(m_epollfd, m_sockfd, EPOLLIN, m_TRIGMode);
...
// {
//     strcpy(m_real_file, doc_root);// 复制根目录路径
//     int len = strlen(doc_root);
//     const char *p = strrchr(m_url, '/');// 查找URL中最后的'/'

//     //处理cgi
//     if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))//2登录3注册
//     {
...
//     //把网站根目录 /home/tinywebserver/root 和目标文件 /welcome.html 拼在一起，得到绝对路径
//     strncpy(m_real_file + len, m_url, FILENAME_LEN - len - 1);
    
//     if (stat(m_real_file, &m_file_stat) < 0)
//         return NO_RESOURCE;//文件不存在 404 Not Found

//     if (!(m_file_stat.st_mode & S_IROTH))
//         return FORBIDDEN_REQUEST;//无权限 403 Forbidden

//     if (S_ISDIR(m_file_stat.st_mode))
//         return BAD_REQUEST;// 是目录而非文件

//     // 使用 mmap！直接把硬盘上的这个文件，映射到操作系统的虚拟内存里！
//     // m_file_address 直接拿到了这块内存的指针
//     // 映射完之后，立刻 close(fd) 关闭文件描述符！
//     // 文件描述符关了，映射的内存还在（直到调用 unmap）。这样极其节约系统宝贵的 fd 资源
...
//     // ==========================================
//     // 仅保留 L(登录) 和 S(注册) 的 CGI 业务分支
//     // ==========================================
//     if (cgi == 1 && (*(p + 1) == 'L' || *(p + 1) == 'S'))
//     {
//         // ... (这部分原封不动，保留原有的解析账号密码逻辑) ...
//         char flag = m_url[1];
...
//     // ==========================================
//     // 已经彻底删除了 0, 1, 5, 6, 7 的分支！
//     // 只要不是登录注册，前端传什么文件名，这里就拼什么文件名！
//     // ==========================================
//     strncpy(m_real_file + len, m_url, FILENAME_LEN - len - 1);

//     if (stat(m_real_file, &m_file_stat) < 0)
//         return NO_RESOURCE; // 警告：如果找不到文件，由于原作者的Bug，这里会导致断开连接！
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    // ==========================================
    if (cgi == 1 && (strcmp(p + 1, "login.cgi") == 0 || strcmp(p + 1, "register.cgi") == 0))
    {
        // 提取账号密码的逻辑保持不变...
        char name[100], password[100];
        int i;
        ...
        //         strcpy(m_url, "/logError.html");
        // }
        // 在提取完明文 name 和 password 之后，立刻计算出加密哈希值！
...
if (strcmp(p + 1, "register.cgi") == 0) 
{
    ...
    strcpy(sql_insert, "INSERT INTO user(username, passwd) VALUES(");
    strcat(sql_insert, "'"); strcat(sql_insert, name); strcat(sql_insert, "', '");
    // 【战术替换】：将存入数据库的明文 password 替换为 hashed_pwd.c_str()
    strcat(sql_insert, hashed_pwd.c_str()); strcat(sql_insert, "')");

    if (users.find(name) == users.end()) {
        m_lock.lock();
        int res = mysql_query(mysql, sql_insert);
        // 【战术替换】：内存字典里存的也是 Hash 值
...
{
    // 【战术替换】：将用户的明文密码加密后，去和数据库/内存里存的 Hash 值进行比对
...
http_conn::LINE_STATUS http_conn::parse_line()
{
    ...
    {
        //一个字符一个字符的解析
        temp = m_read_buf[m_checked_idx];//recv()读取网卡数据后存入m_read_buf内
        if (temp == '\r')
        {
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    m_url = strpbrk(text, " \t");//strpbrk()在参数1(string)里找参数2(char)，text里找\t（空格或者tab），找到则返回指针
    ...
    *m_url++ = '\0';//把那个空格\t强行篡改成了字符串结束符\0,*m_url++先改后移
    ...
    else if (strcasecmp(method, "POST") == 0)
    {
        ...
        // 最后根据验证的对错，决定返回 /welcome.html（成功）还是 /logError.html（失败）
    }
    else
        return BAD_REQUEST;//语法错误
    m_url += strspn(m_url, " \t");//strspn()计算参数1开头连续包含参数2中字符的长度并且跳过
    ...
    *m_version++ = '\0';
    ...
    //协议解析
    if (strcasecmp(m_version, "HTTP/1.1") != 0)//strcasecmp()比较两个字符串是否相等，忽略大小写，返回0代表相同
        return BAD_REQUEST;
    if (strncasecmp(m_url, "http://", 7) == 0)
    {
        m_url += 7;// 跳过 http://    直接定位后面的'/'因为'/'后面的才是有用的，"http://127.0.0.1/index.html"，跳过http://127.0.0.1
        m_url = strchr(m_url, '/');//strchr()在字符串中寻找某一个特定的单个字符
    }
    ...
    if (!m_url || m_url[0] != '/')
        return BAD_REQUEST;
    //当url为/时，显示判断界面
    if (strlen(m_url) == 1)//缺省，比如只有127.0.0.1/，则在后面加上judge.html登录界面
...
http_conn::HTTP_CODE http_conn::process_read()
{
    ...
    HTTP_CODE ret = NO_REQUEST;//主状态机状态，最终的返回值，NO_REQUEST表示请求还不完整，通过下面的操作最终判断主状态机状态
    char *text = 0;
...
{
    HTTP_CODE read_ret = process_read();//真正的请求报文解析函数，返回解析状态，来决定下一步操作
