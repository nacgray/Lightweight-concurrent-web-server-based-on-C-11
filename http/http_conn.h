...
public:
    ...
    enum CHECK_STATE
    {
        ...
        //如果这是一个 POST 请求，状态就会变成CHECK_STATE_CONTENT，准备去读最后的请求体（账号密码）
    };
    ...
    enum HTTP_CODE
    {
        ...
        NO_RESOURCE,
        FORBIDDEN_REQUEST,
        FILE_REQUEST,//客人请求的网页文件不仅存在，而且有权限访问。这是 do_request() 函数在本地找完文件后返回的极佳状态。
        INTERNAL_ERROR,
        CLOSED_CONNECTION
    };
    //从状态机
...
public:
    ...
    void initmysql_result(connection_pool *connPool);//把数据库里的所有用户名和密码提前拉取到本地的 map 里，方便极速校验
...
private:
    ...
    //根据process_read()分析出的结果（比如是200 OK还是404 Not Found），负责生成对应的HTTP响应报文

    //解析请求，拆解HTTP文本
    LINE_STATUS parse_line();//从状态机，找 \r\n，切出一行
    //阶段三2.2：把刚刚切下来的那行文本的首地址返回来
    char *get_line() { return m_read_buf + m_start_line; };
    HTTP_CODE parse_request_line(char *text);//专切第一行。提取出 GET、目标路径 /potato.jpg、HTTP版本号
    HTTP_CODE parse_headers(char *text);//专切请求头。提取出 Host、Content-Length 等信息
    HTTP_CODE parse_content(char *text);//专切请求体。提取出 POST 请求附带的账号密码

    HTTP_CODE do_request();//业务逻辑函数，根据上面函数解析的文本来决定后续操作
    void unmap();//用完图片后，把 mmap 映射的内存释放掉

    //生成响应头
    bool add_response(const char *format, ...);//底层调用C语言极其高级的vsnprintf（可变参数打印），把字符串写进m_write_buf（写缓冲区）
    bool add_status_line(int status, const char *title);//贴上最外面的大标签：HTTP/1.1 200 OK
    bool add_headers(int content_length);//一个打包函数，内部一口气调用下面几个函数
    ...
    bool add_blank_line();//加上一个单独的 \r\n。这代表着：响应头到此结束，下面就是真正的文件数据
    bool add_content(const char *content);//如果出错（比如 404），就直接把错误提示网页的文本拼在后面
...
private:
    ...
    //m_iv[1] 指向文件内容（m_file_address），调用一次函数就能把它们拼接在一起发给网卡
    int m_iv_count;
    int bytes_to_send;
    int bytes_have_send;
    char m_write_buf[WRITE_BUFFER_SIZE];//HTTP 响应头（比如 HTTP/1.1 200 OK），全写在这个数组里
    int m_write_idx;
...
std::string get_salted_hash(const std::string& username, const std::string& password) {
    // 1. 混合材料：用户名(动态盐) + 密码 + 固定的私盐(Pepper)
    std::string salted_pwd = username + password + "wangzaiyi_wangzhiyong_xiaojia_lala_20260220"; 
