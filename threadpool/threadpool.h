...
public:
    ...
    //max_requests是请求队列中最多允许的、等待处理的请求的数量max_request = 10000
...
private:
    ...
    // 然后通过极其精妙的第四个参数 arg，把老板自己（this）偷渡进去，再强转回 threadpool *，最终成功唤醒真正的核心逻辑 pool->run()。
    void run();

private:
    int m_thread_number;        //线程池中的线程数
    int m_max_requests;         //请求队列中允许的最大请求数，可防止黑客网络攻击
    pthread_t *m_threads;       //描述线程池的数组，其大小为m_thread_number，底层就是用 new pthread_t[8]

    std::list<T *> m_workqueue; //半同步/半反应堆 (Half-Sync/Half-Reactive)-2.请求队列 
    //主线程把任务对象（request）通过 append 或 append_p 塞进这个队列，解耦了主线程和工作线程。
    ...
    while (true)
    {
        ...
        if (1 == m_actor_model)
        {
            ...
            if (0 == request->m_state)// 0读
            {
                ...
                {
                    request->improv = 1;
                    connectionRAII mysqlcon(&request->mysql, m_connPool);//向数据库连接池申请一个可用的连接
                    //依旧是第四步内容，执行 HTTP 解析
                    // 拿到任务后，同步执行极其耗时的业务逻辑
